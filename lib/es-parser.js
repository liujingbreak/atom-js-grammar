...
function parseJS(text) {
	...
	var ast;
	try {
		ast = acornjsx.parse(text, {ranges: false, locations: true, allowHashBang: true, plugins: {jsx: true}});
	} catch (err) {
		ast = acornjsx.parse(text, {ranges: false, locations: true, allowHashBang: true, plugins: {jsx: true},
			sourceType: 'module'});
	}
...
function Parser(ast) {
	this.ast = ast;
	this.model = {};
	this.scopeStack = [this.model]; // used for nested node
	this.lastNode = null;
...
Parser.prototype = {
	...
	traversePath2Name: function() {
		...
		var compareCount = 0;
		var index = _.findLastIndex(path, function(node) {
			compareCount++;
	...
	enterHandler: {
		ClassDeclaration: function(n, parent) {
			var className = _.get(n, 'id.name');
	...
	leaveHandler: {
		...
		ClassDeclaration: function(n, p) {
			var classNode = this.scopeStack.pop();
			if (_.get(classNode, 'child.length') > 0) {
				this.addChildNode(classNode);
			}
		}
	}
};
